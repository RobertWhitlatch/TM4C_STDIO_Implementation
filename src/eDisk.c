...
void Timer3_Init(void){
    ...
    TIMER3_TAILR_R = 799999;         // 4) reload value, 10 ms, 80 MHz clock
    TIMER3_TAPR_R = 0;               // 5) bus clock resolution
    TIMER3_ICR_R = 0x00000001;       // 6) clear timer3A timeout flag
    TIMER3_IMR_R = 0x00000001;       // 7) arm timeout interrupt
    NVIC_PRI8_R = (NVIC_PRI8_R&0x00FFFFFF)|0x00000000; // 8) priority 0
    NVIC_EN1_R |= 0x00000008;         // 9) enable interrupt 35 in NVIC
    TIMER3_CTL_R = 0x00000001;       // 10) enable timer3A
}

// Executed every 10 ms
void Timer3A_Handler(void){ 
    TIMER3_ICR_R = 0x00000001;       // acknowledge timer3A timeout
    disk_timerproc();
}
